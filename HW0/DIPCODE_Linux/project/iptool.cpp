...
int main (int argc, char** argv)
{
	...
	while(fgets(str,MAXLEN,fp) != NULL) {
		...
		else if (strncasecmp(pch,"valueThreshold",MAXLEN)==0) {
			/* valueThresholding */
			pch = strtok(NULL, " ");		//holds the first argument, the threshold
			char *val = strtok(NULL, " ");	//holds the second argument, the value
			utility::valueThreshold(src,tgt,atoi(pch), atoi(val));
		}
