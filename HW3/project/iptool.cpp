...
int main (int argc, char** argv)
{
	...
	while(fgets(str,MAXLEN,fp) != NULL) {
        ...
        //get the name of the infile (not including extension)
        std::string inFileName(pch);
        inFileName = inFileName.substr(0, inFileName.length()-4);
		...
		utility::ROI_Processing(src, tgt, inFileName, outFileName, ftype, ROIs, numROI, angle);
        tgt.save(outfile);
	}
