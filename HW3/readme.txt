...
13. Low/High pass filters on grey images: high_low_pass
New function for HW3
Takes in 3 parameters, and integer defining the radius we will use when creating the filter
and 2 floats inside and outside which define what the inside of the radius will be and what the outside will be

*NOTE*: We don't call this function from parameters.txt, but rather pass in "highpass" or "lowpass",
        followed by the radius we want to use. Our processing function parses the string and
        calls high_low_pass with the proper inside/outside values

ex: "lowpass 30" will call high_low_pass with a radius of 30, an inside of 1.0, and an outside of 0.0
ex: "highpass 30" will call high_low_pass with a radius of 30, an inside of 0.0, and an outside of 1.0



14. Low/High pass filters on color images: high_low_pass_hsv
New function for HW3
Takes in 4 parameters, and integer defining the radius we will use when creating the filter,
2 floats inside and outside which define what the inside of the radius will be and what the outside will be.
and an integer representing which channel we are using (H = 0, S = 1, V = 2)

*NOTE*: We don't call this function from parameters.txt, but rather pass in "highpass" or "lowpass",
        followed by the radius we want to use. Our processing function parses the string and
        calls high_low_pass with the proper inside/outside values

ex: "lowpass v 30" will call high_low_pass_hsv with a radius of 30, an inside of 1.0, and an outside of 0.0,
    and perform lowpass only on the V channel
ex: "highpass hsv 30" will call high_low_pass_hsv with a radius of 30, an inside of 0.0, and an outside of 1.0,
    and perform highpass on all channels (hsv)
...
15. bandstop filters on grey images: bandstop
New function for HW3
Takes in 2 parameters, 2 integers describing the two radii to use

ex: "bandstop 10 30" will call bandstop with a radiusA of 10 and a radius B of 30

*NOTE*: The first radius passed in should be the smaller one



16. bandstop filters on color images: bandstop_hsv
New function for HW3
Takes in 3 parameters, 2 integers describing the two radii to use,
and a string denoting which channels to perform bandstop on

ex: "bandstop v 10 30 " will call bandstop with a radiusA of 10 and a radius B of 30 on the V channel
...
17. unsharp masking on grey images: unsharpMasking
New function for HW3
Takes in 2 parameters, an integer radius which is used in the high/low pass filters we use,
and a float which describes the weight we want to give to the highpass filter

ex: "unsharpMasking 30 0.50" will call unsharpMasking with a radius of 30 and an alpha of 0.50
...
-   high_low_pass(int radius, float inside, float outside)
-   bandpass(int radiusA, int radiusB)
-   unsharpMasking(int radius, float alpha)
...
-   high_low_pass_hsv(int radius, float inside, float outside, string channels)
-   bandpass_hsv(int radiusA, int radiusB, string channels)
...
Example 8:
    infile.pgm outfile.pgm 1 0 ROI 0 0 256 256 highpass 30              //correct
    infile.pgm outfile.pgm 1 0 ROI 0 0 256 256 lowpass 30               //correct
    infile.pgm outfile.pgm 1 0 ROI 0 0 256 256 bandstop 10 30           //correct
    infile.pgm outfile.pgm 1 0 ROI 0 0 256 256 unsharpMasking 30 0.50   //correct

    infile.pgm outfile.pgm 1 0 ROI 0 0 256 256 bandstop 40 10           //incorrect
    //the first radius needs to be smaller than the second

    infile.pgm outfile.pgm 1 0 ROI 0 0 256 245 unsharpMasking 30 20     //incorrect
    //the second argument needs to be a floating point number



Example 9:
    infile.ppm outfile.ppm 1 0 ROI 0 0 256 256 highpass v 30            //correct
    infile.ppm outfile.ppm 1 0 ROI 0 0 256 256 lowpass h 30             //correct
    infile.ppm outfile.ppm 1 0 ROI 0 0 256 256 bandstop hsv 10 30       //correct

    infile.ppm outfile.ppm 1 0 ROI 0 0 256 256 lowpass h s V 30         //incorrect
    //the color channels cannot have spaces between them